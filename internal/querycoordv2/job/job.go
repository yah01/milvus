...
func (job *LoadCollectionJob) Execute() error {
	...
	// Make sure the handoff event before this fetches segments can be handled
	job.meta.CollectionManager.PutCollectionWithoutSave(collection)
	...
	err = job.meta.CollectionManager.PutCollection(collection)
	if err != nil {
...
func (job *LoadCollectionJob) PostExecute() {
	if job.Error() != nil &&
		(!job.meta.Exist(job.CollectionID()) || job.meta.GetStatus(job.CollectionID()) == querypb.LoadStatus_Invalid) {
		job.meta.CollectionManager.RemoveCollectionInMemory(job.CollectionID())
...
func (job *LoadPartitionJob) Execute() error {
	...
	job.meta.CollectionManager.PutPartitionWithoutSave(partitions...)
	...
	partitions = lo.Map(partitions, func(partition *meta.Partition, _ int) *meta.Partition {
		partition = partition.Clone()
		partition.Status = querypb.LoadStatus_Loading
		return partition
	})
