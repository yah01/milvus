...
func newBaseSegment(id, partitionID, collectionID int64, shard string, typ SegmentType, version int64, startPosition *internalpb.MsgPosition) baseSegment {
	return baseSegment{
		segmentID:      id,
...
func (s *baseSegment) StartPosition() *internalpb.MsgPosition {
...
func (s *LocalSegment) LoadDeltaData(deltaData *storage.DeleteData) error {
	...
	// The delta logs have been loaded
	lastTs := tss[len(tss)-1]
	if lastTs <= s.LastDeltaTimestamp() {
		return nil
	}
