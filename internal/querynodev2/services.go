...
func (node *QueryNode) LoadSegments(ctx context.Context, req *querypb.LoadSegmentsRequest) (*commonpb.Status, error) {
	...
	node.segmentManager.PutCollectionAndRef(req.GetCollectionID(), req.GetSchema(), req.LoadMeta.GetLoadType())
	defer node.segmentManager.UnrefCollection(req.GetCollectionID())

	if req.GetNeedTransfer() {
		return node.TransferLoad(ctx, req)
	}

	loaded, err := node.loader.Load(ctx, req, segments.SegmentTypeSealed)
	if err != nil {
		return &commonpb.Status{
			ErrorCode: commonpb.ErrorCode_UnexpectedError,
			Reason:    err.Error(),
	...
	node.segmentManager.Put(segments.SegmentTypeSealed, loaded...)
